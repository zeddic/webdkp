...
Site = new (function() {
	...
	this.FlashMessage = function(message, time){
		...
		if (typeof time == "undefined")
			time = 5000;
		setTimeout (function () { Site.HideMessage(); }, time);
	}
	/*==============================================
...
var Util = new (function() {
		...
		if ( document.getElementById(id).style.display == 'none' ) {
			if($(focusAfter)) {
				Effect.SlideDown(id,{duration:duration,
			                  afterFinish:function(){Util.Focus(focusAfter)}});
			}
			else {
				Effect.SlideDown(id,{duration:duration});
			}
	...
	of the text box.
	===================================*/
	this.InsertIntoTextbox = function(textBoxName, toInsert, insertPos, keepFocus) {
		...
		if (element.setSelectionRange) {
			...
			//Put in the tab
			element.value = element.value.substring(0,insertPos) + toInsert + element.value.substring(insertPos,element.value.length);
			//There's no easy way to have the focus stay in the textarea, below seems to work though
			if (keepFocus) {
				setTimeout("var t=$('"+textBoxName+"'); t.focus(); t.setSelectionRange(" + pos_to_leave_caret + ", " + pos_to_leave_caret + ");", 0);
			}
		}
		//Handle IE
	...
	when the Ajax request works or fails.
	===============================================*/
	this.AjaxPartRequest = function(AjaxHandler, partid, data, OnSuccess, OnFailure) {
...
var PageEditor = new (function() {
	...
	this.ShowPartLibrary = function() {
		...
		if (PageEditor.LibraryLoaded) {
			Effect.SlideDown("PartLibrary", {duration:.5});
			return;
		}
	...
	this.HidePartLibrary = function(transport) {
		Effect.SlideUp("PartLibrary", {duration:.5});
	}
	...
	this.UpdateTitle = function(partid) {
		...
		//Send via ajax
		PageEditor.SendUpdate("updateTitle","iid="+partid+"&title="+title,partid, PageEditor.ReloadPart);
	...
	this.ChangeBorder = function(partid, borderid){
		...
		//send ajax request
		PageEditor.SendUpdate("updateBorder","iid="+partid+"&border="+borderid,partid, PageEditor.ReloadPart);
	...
	this.UpdateCustomOption = function(partid, optionid){
		...
		PageEditor.SendUpdate("UpdateCustomOption","partid="+partid+"&optionid="+optionid+"&value="+value,partid);
	}
	...
	this.AddPartCallback = function(transport){
		...
		if(partid == NaN || partid == "NaN" || partid == "" || partid == 0 ) {
			alert("An error was encountered while adding the part. The ajax request returned '0' as the part id");
			Site.HideLoading();
			return;
		}
		//part was added.
	...
	this.DeletePart = function(partid){
		...
		Effect.Fade(divId);
		setTimeout(function(){PageEditor.RemovePartFromDOM(partid);},1000);
	...
	this.RemovePartFromDOM = function (partid){
		var divId = "part_"+partid;
		PageEditor.SendUpdate("DeletePart","pageid="+Site.PageId+"&partid="+partid);
	...
	this.ShowPartEditor = function(partid,view){
		...
		Site.GetPart(partid, PageEditor.LoadEditPartCallback, "v"+partid+"="+view, "0");
	}
	...
	this.ShowPartButtons = function(partid) {
		...
		if($("EditPageBorderButtonsEdit"+partid).visible() )
			return;
	...
	this.Setup = function() {
		...
		if($("area1_container")){
			...
			{dropOnEmpty:true,handle:"handle",constraint:false,onUpdate:function(){if(PageEditor.AreaUpdate != "undefined"){PageEditor.AreaUpdate('area1')}},
			revert:true,
			containment:["area1_container","area2_container","area3_container","area4_container","area5_container"]});
		}

		if($("area2_container")){

		Sortable.create("area2_container",
			{dropOnEmpty:true,handle:"handle",constraint:false,onUpdate:function(){if(PageEditor.AreaUpdate != "undefined"){PageEditor.AreaUpdate('area2')}},
			revert:true,
			containment:["area1_container","area2_container","area3_container","area4_container","area5_container"]});
		}
		...
		Sortable.create("area3_container",
			{dropOnEmpty:true,handle:"handle",constraint:false,onUpdate:function(){if(PageEditor.AreaUpdate != "undefined"){PageEditor.AreaUpdate('area3')}},
			revert:true,
			containment:["area1_container","area2_container","area3_container","area4_container","area5_container"]});
		}
		...
		Sortable.create("area4_container",
			{dropOnEmpty:true,handle:"handle",constraint:false,onUpdate:function(){if(PageEditor.AreaUpdate != "undefined"){PageEditor.AreaUpdate('area4')}},
			revert:true,
			containment:["area1_container","area2_container","area3_container","area4_container","area5_container"]});
		}
		...
		Sortable.create("area5_container",
			{dropOnEmpty:true,handle:"handle",constraint:false,onUpdate:function(){if(PageEditor.AreaUpdate != "undefined"){PageEditor.AreaUpdate('area5')}},
			revert:true,
			containment:["area1_container","area2_container","area3_container","area4_container","area5_container"]});
		}
	}
