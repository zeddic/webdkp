...
<input id="saveEditChanges" class="mediumButton" style="width:150px" type="button" value="Save Changes" onclick="BBCodePart.SaveChanges()">
<input class="mediumButton" type="button" value="Back" onclick="document.location='<?=$PHP_SELF?>?editpage=1'" >
<br /><br />

<div class="editTabs">
	<ul>
		<li class="<?=(!$uploadedImage?"selected":"back")?>" id="htmlTab"><a href="javascript:BBCodePart.ShowTab('html')"><span>Editor</span></a></li>
		<li class="back" id="previewTab"><a href="javascript:BBCodePart.ShowTab('preview')"><span>Preview</span></a></li>
		<li class="back" id="imageTab"><a href="javascript:BBCodePart.ShowTab('image')"><span>Images</span></a></li>
		<li class="<?=($uploadedImage?"selected":"back")?>" id="uploadTab"><a href="javascript:BBCodePart.ShowTab('upload')"><span>Upload</span></a></li>
	</ul>
...
<div id="htmlTabContent" <?=($uploadedImage?"style='display:none'":"")?>>
	...
	<textarea name="content"  id="content" style="width:100%;height:600px;padding:2px" onclick="BBCodePart.OnChange();BBCodePart.OnContentClick(this)"
		onkeydown="BBCodePart.ConvertTab(event,this)"
><?=($tempcontent!=""?$tempcontent:$content->content)?></textarea>
	</form>
</div>
...
<div id="imageTabContent" style="display:none">
	...
	<div class="photo" id="image_<?=$image->id?>" onmouseover="Util.Show('toolbar_<?=$image->id?>')" onmouseout="Util.Hide('toolbar_<?=$image->id?>')">
		...
		<div class="imageToolbar" id="toolbar_<?=$image->id?>" style="display:none">
			<span class="imageToolbarIcon" style="float:right" onclick="BBCodePart.DeleteImage(this,'<?=$image->id?>')"><img src="<?=$directory?>images/delete.png" title="Delete Image" ></span>
			<span class="imageToolbarIcon" style="float:left" onclick="BBCodePart.InsertImage('<?=$image->id?>','l','<?=$image->originalname?>')"><img src="<?=$directory?>images/photolarge.png" title="Insert Large Image" ></span>
			<span class="imageToolbarIcon" style="float:left" onclick="BBCodePart.InsertImage('<?=$image->id?>','m','<?=$image->originalname?>')"><img src="<?=$directory?>images/photomedium.png" title="Insert Medium Image"></span>
			<span class="imageToolbarIcon" style="float:left" onclick="BBCodePart.InsertImage('<?=$image->id?>','s','<?=$image->originalname?>')"><img src="<?=$directory?>images/photosmall.png" title="Insert Small Image"></span>
		</div>
	</div>
	<?php }} ?>
	</div>
