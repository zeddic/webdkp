...
DKPManage = new (function() {
	...
	this.ShowAlts = function(user) {
		$("altsTitle").innerHTML = "Alts For "+user.player;
		Util.Hide('TableContent');
		Util.Hide('AwardButton');
		Util.Show('Alts');
	}

	this.HideAlts = function() {
		Util.Hide('Alts');
		Util.Show('TableContent');
		Util.Show('AwardButton');
	}
	...
	this.PreparePlayerTable = function() {

		if (DKPManage.playerTableReady) {
			DKPManage.playertable.Redraw();
			return;
		}
		DKPManage.playertable = new SimplePlayerSelectTable("playertable");
		DKPManage.playertable.EnablePaging(100);

		for ( var i = 0 ; i < table.items.length ; i++ ) {
			if(!table.items[i].deleted) {
				DKPManage.playertable.Add(table.items[i]);
			}
		}

		DKPManage.playertable.CalculatePagingInfo();

		DKPManage.playertable.DrawOnLoad();

		DKPManage.playerTableReady = true;
	}

	this.ProcessItemAward = function() {
		var players = [];
		if( DKPManage.playertable && DKPManage.zerosum )
			players = DKPManage.playertable.GetSelectedItems();
		if( players.length == 0 && DKPManage.zerosum ) {
			for ( var i = 0 ; i < DKPManage.playertable.items.length ; i++ )
				players.push(DKPManage.playertable.items[i].userid);
		}
	...
	this.CreateItemAwardCallback = function(transport)
	{
		...
		//on success
			...
			//deduct the points from the player on the gui
			DKPManage.dkp = DKPManage.dkp * -1;
			DKPManage.UpdatePlayerDKP(DKPManage.playerid, DKPManage.dkp);

			//if zerosum is turned on, an equal but opposite amount must
			//be given to everyone
			if( DKPManage.zerosum ) {

				if (DKPManage.players.length == 0) {
					DKPManage.players = [];
					for ( var i = 0 ; i < DKPManage.playertable.items.length ; i++ ) {
						DKPManage.players.push(DKPManage.playertable.items[i].userid);
					}
				}
				var numberOfPlayers = DKPManage.players.length;
				if( numberOfPlayers != 0 ){
					var dkp = ((DKPManage.dkp)*-1) / numberOfPlayers;
					for ( var i = 0 ; i < DKPManage.players.length ; i++ ) {
						DKPManage.UpdatePlayerDKP(DKPManage.players[i], dkp);
					}
				}
			}
	...
	this.ProcessAward = function() {
		var players = DKPManage.playertable.GetSelectedItems();
	...
	this.CreateAwardCallback = function(transport)
	{
		...
		//on success
			...
			for ( var i = 0 ; i < DKPManage.players.length ; i++ ) {
				DKPManage.UpdatePlayerDKP(DKPManage.players[i], DKPManage.dkp);
			}
	...
	{
		Util.Hide("AwardContentFinished");
		Util.Show("TableContent");
		Util.Show('AwardButton');
	}

	this.UpdatePlayerDKP = function(userid, toadd)
	{
		for ( var i = 0 ; i < table.items.length ; i++ ) {
			if( table.items[i].userid == userid ) {
				var current = parseFloat(table.items[i].dkp);
				table.items[i].dkp = current + toadd;
				table.items[i].dkp = Math.round(table.items[i].dkp*100)/100;
				table.RecreateRow(i);
				break;
			}
		}
	...
	this.AddPlayerToInput = function(user)
	{
		//add the player to the selection table
		if (DKPManage.playertable) {
			//add the user to memory
			DKPManage.playertable.Add(user);
			//generate a row
			var row = DKPManage.playertable.GetRow(DKPManage.playertable.items.length - 1 );
			//add the row to memory
			DKPManage.playertable.rowObjects[DKPManage.playertable.items.length-1] = row;
		}

		//add the player to the player drop down
		var option = Builder.node('option',{value:user.userid},user.player);
		$("userdropdown").appendChild(option);
	}

	this.RemovePlayerFromInput = function(userid)
	{
		//remove the player from the selection table
		if (DKPManage.playertable) {
			for ( var i = 0 ; i < DKPManage.playertable.items.length ; i++ ) {
				if(DKPManage.playertable.items[i].userid == userid){

					DKPManage.playertable.items[i].deleted = true;
					//DKPManage.playertable.tableBody.splice(i,1);
					//DKPManage.playertable.items.splice(i,1);
					//var row = DKPManage.playertable.rowObjects[i];
					//DKPManage.playertable.tableBody.removeChild(row);
				}
			}
		}
		//remove the player from the drop down
		var selection = $("userdropdown");
		for ( var i = 0 ; i < selection.options.length ; i++ ) {
			if (selection.options[i].value == userid) {
				selection.removeChild(selection.options[i]);
			}
		}
	}

	this.RenamePlayerInInput = function(userid, name)
	{
		//update player in the selection table
		if (DKPManage.playertable) {
			for ( var i = 0 ; i < DKPManage.playertable.items.length ; i++ ) {
				if (DKPManage.playertable.items[i].userid == userid) {
					DKPManage.playertable.items[i].player = name;
					DKPManage.playertable.RecreateRow(i);
				}
			}
		}
		//update the player in the drop down
		var selection = $("userdropdown");
		for ( var i = 0 ; i < selection.options.length ; i++ ) {
			if (selection.options[i].value == userid) {
				selection.options[i].innerHTML = name;
			}
		}
	}
...
var ManageDKPTable = Class.create(ManualPageTable, {
	...
	GetRow: function(i) {
		...
		if( this.canEditPlayer ) {
			...
			Event.observe(editLink,'click', this.OnAltClick.bindAsEventListener(this, i));
	...
	SaveActiveChanges: function() {
		...
		if (this.items[i].playername != newname) {
			DKPManage.RenamePlayerInInput(this.items[i].userid, newname);
		}
	...
	OnDeleteItemCallback: function(transport) {
		...
		if(!result[0]) {
			...
			DKPManage.RemovePlayerFromInput(this.items[i].userid);
	...
	OnAddPlayerCallback: function(transport) {
		...
		if(!result[0]) {
			...
			DKPManage.AddPlayerToInput(item);
		}
	},
	/*================================================
...
/*var ManageDKPTable2 = Class.create(ManualPageTable, {

	GetRow: function(i) {
		var row = Builder.node('tr');
		row.appendChild(Builder.node('td',{},this.items[i].name));
